      ...
      allIssues.map(async (issue) => {
        try {
          ...
          try {
            ...
            core.warning(`Issue #${issue.number} のタイムラインイベント取得に失敗: ${timelineError.message}. listEventsで再試行します。`);
          }
      ...
      events: events.length > 0 ? events.map(event => {
        ...
        if (eventAny.changes) {
          try {
            ...
            if (typeof core.debug === "function") {
              core.debug(`Issue event changes のクローンに失敗: ${debugMessage}`);
            }
