...
export function TrendAnalysis({
  ...
  statistics,
  palette,
}: TrendAnalysisProps): ReactElement {
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          const data = params[0];
          return `${data.name}<br/>${data.seriesName}: ${formatNumber(
            data.value
          )}${METRICS[metric].unit}`;
        },
      },
  ...
  return (
    <div className={styles.analysisTab}>
      ...
      <div className={styles.insightArea}>
        <h4>📊 分析結果</h4>
        <p>
          {METRICS[metric].label}の直近{PERIOD_OPTIONS[period].label}
          のトレンドを表示しています。
          {stats && chartData.length > 0 && (
            <>
              {" "}
              平均は{formatNumber(stats.mean)}
              {METRICS[metric].unit}、 中央値は{formatNumber(stats.median)}
              {METRICS[metric].unit}です。
            </>
          )}
        </p>
      </div>
...
export function DistributionAnalysis({
  ...
  statistics,
  palette,
}: DistributionAnalysisProps): ReactElement {
  const [metric, setMetric] = useState<MetricKey>("leadTime");
  const [chartType, setChartType] =
    useState<DistributionChartType>("histogram");
  const [period, setPeriod] = useState<PeriodOption>("30d");
  ...
  return (
    <div className={styles.analysisTab}>
      <div className={styles.analysisControls}>
        ...
        <div className={styles.controlGroup}>
          ...
          <select
            value={chartType}
            onChange={(e) =>
              setChartType(e.target.value as DistributionChartType)
            }
          >
      ...
      <div className={styles.insightArea}>
        <h4>📊 分析結果</h4>
        <p>
          {METRICS[metric].label}の分布を
          {chartType === "histogram" ? "ヒストグラム" : "箱ひげ図"}
          で表示しています。
          ...
          {excludeOutliers &&
            ...
            ` IQR法により${
              statistics.anomalies.iqrOutliers.filter((o) => o.isOutlier).length
            }件の外れ値を除外しています。`}
        </p>
      </div>
    </div>
...
export function CorrelationAnalysis({
  ...
  statistics,
  palette,
}: CorrelationAnalysisProps): ReactElement {
    ...
    metrics.forEach((m1) => {
      ...
      metrics.forEach((m2) => {
        if (m1 === m2) {
          ...
          const factor = statistics.correlations.topFactors[m1]?.find(
            (f) => f.factor === m2
          );
    ...
    if (chartType === "scatter") {
      return {
        ...
        tooltip: {
          ...
          formatter: (params: any) => {
            ...
            return (
              `${METRICS[xAxisMetric].label}: ${params.value[0].toFixed(
                2
              )}<br/>` +
      ...
      return {
        tooltip: {
          position: "top",
          formatter: (params: any) => {
            return `${labels[params.data[0]]} × ${
              labels[params.data[1]]
            }<br/>相関係数: ${params.data[2]}`;
          },
        },
    ...
    <div className={styles.analysisTab}>
      <div className={styles.analysisControls}>
        <div className={styles.controlGroup}>
          ...
          <select
            value={chartType}
            onChange={(e) =>
              setChartType(e.target.value as CorrelationChartType)
            }
          >
        ...
        {chartType === "scatter" && (
          <>
            <div className={styles.controlGroup}>
              ...
              <select
                value={xAxisMetric}
                onChange={(e) => setXAxisMetric(e.target.value as MetricKey)}
              >
                {Object.values(METRICS).map((m) => (
                  <option key={m.key} value={m.key}>
                    {m.label}
            ...
            <div className={styles.controlGroup}>
              ...
              <select
                value={yAxisMetric}
                onChange={(e) => setYAxisMetric(e.target.value as MetricKey)}
              >
                {Object.values(METRICS).map((m) => (
                  <option key={m.key} value={m.key}>
                    {m.label}
      ...
      <div className={styles.tableArea}>
        <table className={styles.dataTable}>
          ...
          <tbody>
            {statistics &&
              Object.entries(statistics.correlations.topFactors).map(
                ([metric, factors]) =>
      ...
      <div className={styles.insightArea}>
        ...
        {chartType === "heatmap" ? (
          ...
          <p>
            {METRICS[xAxisMetric].label}と{METRICS[yAxisMetric].label}
            の関係を散布図で表示しています。
            赤い線は回帰線（最小二乗法）を示します。
          </p>
        )}
  ...
  return (
    <div className={styles.analysisTab}>
      <div className={styles.analysisControls}>
        ...
        <div className={styles.controlGroup}>
          ...
          <select
            value={segmentType}
            onChange={(e) =>
              setSegmentType(e.target.value as "label" | "assignee")
            }
          >
      ...
      <div className={styles.insightArea}>
        <h4>📊 分析結果</h4>
        <p>
          {segmentType === "label" ? "ラベル" : "担当者"}別の
          {METRICS[metric].label}を表示しています。
          セグメント間の差異を確認できます。
        </p>
      </div>
    </div>
