...
import { calculateCycleTimePercentile } from "./index.js";
...
describe("calculateCycleTimePercentile", () => {
  it("複数のメトリクスから durationMs のパーセンタイルを計算する", () => {
    ...
    const result = calculateCycleTimePercentile(metrics, 50);
  ...
  it("0パーセンタイルは最小値を返す", () => {
    ...
    const result = calculateCycleTimePercentile(metrics, 0);
  ...
  it("100パーセンタイルは最大値を返す", () => {
    ...
    const result = calculateCycleTimePercentile(metrics, 100);
