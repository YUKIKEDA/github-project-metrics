...
export interface KurtosisOptions {
  ...
  mean?: number;
  /**
   * サンプルの過剰尖度（不偏推定量）を返す場合は true（既定値）、
   * 母集団の尖度を返す場合は false を指定します。
   */
  unbiased?: boolean;
  /**
   * true（既定値）の場合は過剰尖度（Fisher 定義）を返し、false の場合は通常の尖度を返します。
   */
  excess?: boolean;
}

export function kurtosis(values: ReadonlyArray<number>, options: KurtosisOptions = {}): number {
