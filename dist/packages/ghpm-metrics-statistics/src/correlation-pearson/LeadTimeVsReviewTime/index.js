...
/**
 * LeadTimeMetric と ReviewTimeMetric のピアソン相関係数を計算します。
 *
 * @param leadTimes - LeadTimeMetric 配列
 * @param reviewTimes - ReviewTimeMetric 配列
 * @returns 相関係数（-1 から 1 の範囲）。計算できない場合は undefined を返します。
 */
export function calculateLeadTimeVsReviewTimeCorrelationPearson(leadTimes, reviewTimes) {
    ...
    try {
        return correlationPearson(leadTimeValues, reviewTimeValues);
    }
    catch {
        return undefined;
    }
